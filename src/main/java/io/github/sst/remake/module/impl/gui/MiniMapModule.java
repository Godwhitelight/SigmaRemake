    ...
    public void onTick(ClientPlayerTickEvent ignoredEvent) {
        ...
        while (iterator.hasNext()) {
            ...
            int distance = cache.chunk.getPos()
                    .method_24022(new ChunkPos((client.player.getBlockPos().getX() >> 4), (client.player.getBlockPos().getZ() >> 4)));
            if (distance > 7) {
                iterator.remove();
            }
        }
        ...
        playerChunkOffsetX =
                (client.player.getX() - (client.player.getBlockPos().getX() >> 4) * 16.0) / 16.0;
        playerChunkOffsetZ =
                (client.player.getZ() - (client.player.getBlockPos().getZ() >> 4) * 16.0) / 16.0;
    ...
    private List<WorldChunk> collectNearbyChunks() {
        ...
        for (int x = -mapChunkRadius / 2; x < mapChunkRadius / 2; x++) {
            for (int z = -mapChunkRadius / 2; z < mapChunkRadius / 2; z++) {
                chunks.add(client.world.getChunk((client.player.getBlockPos().getX() >> 4) + x, (client.player.getBlockPos().getZ() >> 4) + z));
            }
        }
        return chunks;
    }
